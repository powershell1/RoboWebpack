...
export class ContinuousFlyout extends Blockly.VerticalFlyout {
  scrollPositions: Array<{name: string, position: Blockly.utils.Coordinate}>;
  scrollTarget: number | null;
  scrollAnimationFraction: number;
  recyclingEnabled_: boolean;
  labelGaps: Array<number> = [];
   ...
   * @private
   */
  getParentToolbox_(): ContinuousToolbox {
    const toolbox = this.targetWorkspace.getToolbox() as ContinuousToolbox;
   ...
   * @package
   */
  getCategoryScrollPosition(name: string): Blockly.utils.Coordinate {
    ...
    return new Blockly.utils.Coordinate(0, 0);
  }
   ...
   * @override
   */
  show(flyoutDef: string | Blockly.utils.toolbox.FlyoutDefinition) {
   ...
   * @protected
   */
  blockIsRecyclable_(block: Blockly.BlockSvg): boolean {
    ...
    for (const input of block.inputList) {
      ...
      if (input.connection) {
        const targetBlock =
          ...
          (input.connection.targetBlock() as Blockly.BlockSvg);
   ...
   */
  layout_(contents: Array<FlyoutItem>, gaps: Array<number>) {
